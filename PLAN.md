...
## 2026-09-14 이어서 검증한 결과

- 운영 검증 완료: Cloudflare의 실제 cron에서 `empty_stale_queue` 확인 후 새 실행을 요청했고, GitHub 실행 `34844306666`이 21:36:58 KST에 성공했습니다. 공개 시세 생성 시각 21:36:21 KST 및 신고가 8개 게시를 확인하고 로컬에도 동기화했습니다.
- 후속 배포: 신고가 자동 수집과 최신 거래일 자동 이동을 GitHub Pages에 반영했습니다. 9월 14일 한국·일본·대만 원본이 자동 수집되어 총 8개 일별 리포트가 게시되었고 시세 생성 시각도 21:29 KST로 갱신됐습니다.
- 예약 실행에서 작업 0개의 오래된 대기 실행이 일반·강제 취소 모두 409를 반환하는 문제를 확인했습니다. 작업 수와 전체 실행 상태를 재확인하고 동일한 오래된 대기만 남았을 때 새 실행을 허용하도록 보완했습니다. 관련 예약 실행 테스트 42개 통과.
- 로컬 검증 완료: Python 테스트 72개, 예약 실행 테스트 39개, TypeScript 및 Vite 프로덕션 빌드, 실제 Chromium 브라우저 자동 갱신 검증 통과.
- 브라우저 검증은 `frontend`에서 빌드 후 `npm.cmd run test:browser`로 재현합니다. 메인·종목 상세·신고가의 자동 반영, 오류 시 기존 기록 유지 및 복구를 확인합니다.
- 2026-09-14 21:03 KST경 공개 사이트 확인: 시세 생성 시각은 같은 날 15:08 KST, 신고가 게시 리포트는 0개이며 갱신 상태 필드도 없습니다. 로컬의 신고가 원본 5개 및 수집·예약 복구 변경은 아직 공개 사이트에 반영되지 않았습니다.
- 21:03 점검 당시 남아 있던 Pages·Cloudflare 배포와 실제 예약 실행 검증은 위 후속 작업에서 완료했습니다.

## 2026-09-13 일별 신고가 및 예약 실행 복구

이 절을 이전 설계보다 우선합니다.

- 시세는 기존 5분 갱신 목표를 유지하고, 신고가는 시장 현지시간 장 마감 후 거래일당 한 번 수집합니다. 브라우저의 1분 확인은 게시된 결과 확인입니다.
- 한국·일본 16:30, 대만 14:30, 미국 17:00, 유럽(파리) 19:00부터 수집합니다. 현지 시간대의 서머타임을 반영합니다.
- 검증된 원본이 있으면 재수집하지 않습니다. 실패/거래일 미확인은 30분 뒤 재시도하며, 다음 장이 시작되면 이전 날짜 실시간 스냅샷 수집을 중단합니다.
- 자료원은 TradingView 주식 스크리너입니다. 당일 고가와 52주/역대 고가 및 실제 일봉 날짜를 검증합니다. 신고가 배경을 추측하지 않으며 자동 수집 자료에는 배경 미확인을 표시합니다.
- 중국은 현재 자료원이 베이징 거래소를 포함하지 않아 자동 수집 미지원으로 명시합니다. 검증된 일별 자료 수동 등록은 계속 지원합니다.
- 원본 및 재시도 상태를 시세 수집·프런트엔드 빌드 전에 저장합니다. 시세 수집 전체 실패 시 기존 가격과 시각을 보존하고 신고가 게시를 진행할 수 있습니다.
- 오래된 GitHub queued/pending 실행으로 예약 실행이 영구 중단되지 않도록 45분 이상 변화 없는 대기를 확인 후 한 건씩 취소 요청하고 다음 cron에서 재확인합니다.

## 2026-09-12 갱신 보완

아래의 초기 설계보다 이 절과 README의 현재 동작을 우선합니다.

- GitHub의 5분 cron 설정과 달리 실제 실행 기록은 약 2~4시간 간격으로 확인됐습니다.
- PC와 무관하게 Cloudflare Cron에서 5분마다 기존 GitHub 워크플로를 호출하는 스케줄러를 추가합니다. 외부 계정 연결·비밀값 등록·배포 후 활성화하며, GitHub cron은 예비 경로로 유지합니다.
- 모든 종목을 매번 조회하고, 일부 조회 실패는 마지막 정상 가격·조회 시각을 보존하면서 `stale` 상태로 표시합니다. 전체 실패는 게시를 중단합니다.
- 메인·상세·신고가 모두 1분마다 JSON을 재확인하고, 탭 복귀·연결 복구 시 즉시 확인합니다. 배포 파일 확인 버튼은 외부 시세를 직접 수집하는 버튼과 구분합니다.
- 브라우저 확인 시각, 데이터 생성 시각, 실제 시세 기준 시각을 구분합니다. 20분 넘게 새 배포 데이터가 없으면 갱신 지연을 표시합니다.
- 신고가 종목·사유는 검증된 일별 원본 자료를 등록하는 기존 방식을 유지합니다.
- 후속 운영 점검에서 Cloudflare가 지원하지 않는 `redirect: "error"` 때문에 실행 요청이 실패하는 문제를 재현하고 `manual`로 수정했습니다. 3xx 차단 회귀 테스트와 실제 Workers 런타임 검증을 추가로 수행했습니다.
- `scripts/check_refresh_status.py`로 공개 배포 데이터의 생성 시각·경과 시간을 확인하며, 20분 초과·조회 실패를 종료 코드로 구분합니다.
