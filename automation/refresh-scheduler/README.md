...
생성 시각과 마지막 변경 시각이 모두 45분 이상 지난 `queued`·`pending` 실행은 상태별 최대 100건 중 가장 오래된 한 건을 골라 ID·워크플로·브랜치·상태·시각을 개별 조회로 재검증합니다. 일반 취소가 409이면 다시 검증한 뒤 강제 취소를 한 번 요청합니다. 취소가 수락되면 다음 예약 실행에서 재확인합니다. 실행 중인 빌드나 승인 대기는 취소하지 않습니다.

두 취소 API가 모두 409를 반환하면 대상의 실제 작업이 0개인지 확인하고, 모든 실행 상태를 다시 조회합니다. 동일한 오래된 대기 실행 외에 다른 실행이 없을 때만 새 빌드를 요청합니다. 작업이 있거나 조회 실패·누락·상태 변경이 있으면 건너뜁니다. 2026-09-14에는 27시간 이상 멈춘 작업 0개의 대기 실행이 두 취소 요청을 모두 거부하는 현상을 실제 확인했습니다. 정상 요청은 API 6회, 취소 복구는 최대 9회, 빈 대기 실행을 건너뛰는 경우는 최대 16회입니다. 조회와 요청 사이의 경합은 GitHub 워크플로의 `concurrency` 설정으로 제어합니다.
